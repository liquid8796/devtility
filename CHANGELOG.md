...
### Changed
- Đổi tên category **Technology → Developer Tools** (header, trang chủ, sidebar, breadcrumb, search palette). Giữ nguyên id/URL `/tools/technology/*` để bookmark và SEO không gãy; tagline category cập nhật phản ánh đủ 13 công cụ thay vì chỉ code editor.

## [1.5.0] — 2026-07-23

### Added
- **Search bar tìm công cụ** trên header: command palette kiểu Ctrl/⌘+K, tìm không cần gõ dấu tiếng Việt ("mui gio" → "Múi giờ") trên tên/từ khóa/mô tả cả hai ngôn ngữ, kết quả nhóm theo category kèm highlight phần khớp, điều hướng hoàn toàn bằng bàn phím (↑↓ / Enter / Esc), chuẩn ARIA combobox + listbox. Query rỗng liệt kê toàn bộ 24 công cụ để duyệt nhanh.

## [1.4.0] — 2026-07-23

### Added
- **Code Editor — tham số dòng lệnh (args)**: ô nhập args kiểu shell (hỗ trợ nháy kép/nháy đơn cho giá trị có khoảng trắng). Engine tự công bố khả năng qua `/api/execute/runtimes` (`capabilities`): Piston self-host nhận args cho mọi ngôn ngữ; Wandbox chỉ Python (đã kiểm chứng API thật — `runtime-option-raw` với Java bị chèn trước tên class thành JVM options); ô nhập tự disable kèm giải thích khi engine không hỗ trợ.
- **Code Editor — thanh metrics sau mỗi lần chạy**: tổng thời gian (engine báo hoặc server đo), CPU time + bộ nhớ đỉnh (khi engine báo — Piston ≥ 3.1), kích thước output (stdout + stderr), exit code / signal màu theo trạng thái, nhãn runtime. JavaScript trong trình duyệt hiển thị thời gian + kích thước output.
- API `/api/execute`: nhận `args` (validate: ≤ 16 tham số, mỗi cái ≤ 512 B, tổng ≤ 4 KB, cấm xuống dòng; 400 kèm thông báo rõ khi engine không hỗ trợ args cho ngôn ngữ đó), trả thêm `durationMs`; `StageResult` thêm `wallTimeMs`/`cpuTimeMs`/`memoryBytes`.
